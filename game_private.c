...
{
    if (dir == HERE)
        return _inside(g, i, j);
    if (dir == UP)
        return _inside(g, i - 1, j);
    if (dir == DOWN)
        return _inside(g, i + 1, j);
    if (dir == LEFT)
        return _inside(g, i, j - 1);
    if (dir == RIGHT)
        return _inside(g, i, j + 1);
    if (dir == UP_LEFT)
        return _inside(g, i - 1, j - 1);
    if (dir == UP_RIGHT)
        return _inside(g, i - 1, j + 1);
    if (dir == DOWN_LEFT)
        return _inside(g, i + 1, j - 1);
...
{
    if (dir == HERE)
        return _test(g, i, j, s);
    if (dir == UP)
        return _test(g, i - 1, j, s);
    if (dir == DOWN)
        return _test(g, i + 1, j, s);
    if (dir == LEFT)
        return _test(g, i, j - 1, s);
    if (dir == RIGHT)
        return _test(g, i, j + 1, s);
    if (dir == UP_LEFT)
        return _test(g, i - 1, j - 1, s);
    if (dir == UP_RIGHT)
        return _test(g, i - 1, j + 1, s);
    if (dir == DOWN_LEFT)
        return _test(g, i + 1, j - 1, s);
...
bool _neigh(cgame g, uint i, uint j, square s, bool diag)
{
    ...
    // orthogonally
    if (_test_neigh(g, i, j, s, UP))
        return true;
    if (_test_neigh(g, i, j, s, DOWN))
        return true;
    if (_test_neigh(g, i, j, s, LEFT))
        return true;
    if (_test_neigh(g, i, j, s, RIGHT))
        return true;

    // diagonally
    if (diag) {
        if (_test_neigh(g, i, j, s, UP_LEFT))
            return true;
        if (_test_neigh(g, i, j, s, UP_RIGHT))
            return true;
        if (_test_neigh(g, i, j, s, DOWN_LEFT))
            return true;
        if (_test_neigh(g, i, j, s, DOWN_RIGHT))
            return true;
    }
...
uint _neigh_size(cgame g, uint i, uint j, bool diag)
{
    ...
    // diagonally
    if (diag) {
        if (_inside_neigh(g, i, j, UP_LEFT))
            size++;
        if (_inside_neigh(g, i, j, UP_RIGHT))
            size++;
        if (_inside_neigh(g, i, j, DOWN_LEFT))
            size++;
        if (_inside_neigh(g, i, j, DOWN_RIGHT))
            size++;
    }
