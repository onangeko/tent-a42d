...
int game_check_move(cgame g, uint i, uint j, square s)
{
    ...
    if ((s == TENT) && _neigh(g, i, j, TENT, !g->diagadj))
        return LOSING;
    ...
    if (s == TENT) {
        if (nb_tent_squares_row > g->nb_tents_row[i])
            return LOSING;
        if (nb_tent_squares_col > g->nb_tents_col[j])
            return LOSING;
    }
    ...
    // just check this rule, if I play TENT
    if (s == TENT) {
        if (nb_tent_squares_all > _nb_squares(g, TREE))
            return LOSING;
    }
    ...
    if ((s == TENT) && !_neigh(g, i, j, TREE, false))
        return LOSING;
    ...
    if (s == GRASS) {
        if (_test_neigh(g, i, j, TREE, UP) && ((_neigh_count(g, i - 1, j, GRASS, false) + delta_grass_squares) == _neigh_size(g, i - 1, j, false)))
            return LOSING;
        if (_test_neigh(g, i, j, TREE, DOWN) && ((_neigh_count(g, i + 1, j, GRASS, false) + delta_grass_squares) == _neigh_size(g, i + 1, j, false)))
            return LOSING;
        if (_test_neigh(g, i, j, TREE, LEFT) && ((_neigh_count(g, i, j - 1, GRASS, false) + delta_grass_squares) == _neigh_size(g, i, j - 1, false)))
            return LOSING;
        if (_test_neigh(g, i, j, TREE, RIGHT) && ((_neigh_count(g, i, j + 1, GRASS, false) + delta_grass_squares) == _neigh_size(g, i, j + 1, false)))
            return LOSING;
    }
...
bool game_is_over(cgame g)
{
    ...
    // check rule 2)
    for (uint i = 0; i < g->nb_rows; i++)
        if (_nb_squares_row(g, i, TENT) != g->nb_tents_row[i])
            return false;

    for (uint j = 0; j < g->nb_cols; j++)
        if (_nb_squares_col(g, j, TENT) != g->nb_tents_col[j])
            return false;

    // check rule 3)
    if (_nb_squares(g, TREE) != _nb_squares(g, TENT))
        return false;
